...
def mySchedulerFunction(clientSocket):
    ...
    if mReceived.url_func not in funcCallTable:
            funcCallTable[mReceived.url_func] = mReceived.PID
            start = time.time()
            ...
            futures[(mReceived.url_func, mReceived.PID)] = future
            ...
            timestampWait = funcCallTable[mReceived.url_func]
            start = time.time()
            while (mReceived.url_func, timestampWait) not in futures:
                originalSleep(5/1000000.0)
            end = time.time()
            mapPIDtoIdle[mReceived.PID] += end - start
            clientSocket.sendall(pickle.dumps(futures[(mReceived.url_func, timestampWait)]))
            clientSocket.close()
            numRunning = 0
            for index in range(len(requestQueue)):
                if(mapPIDtoStatus[requestQueue[index]] == "running"):
