...
def mySchedulerFunction(clientSocket):
    ...
    global mapPIDtoIdle
    global numCores
    global mapPIDtoSocket
    global funcCallTable
    ...
    if mReceived.url_func == "http://192.168.5.1:3233/api/v1/namespaces/guest/actions/funcC":
        start = time.time()
        ...
        if mReceived.url_func not in funcCallTable:
            funcCallTable[mReceived.url_func] = []
            funcCallTable[mReceived.url_func].append(mReceived.PID)
            mapPIDtoSocket[mReceived.PID] = clientSocket
            start = time.time()
            future = originalRequest(mReceived.url_func, params=mReceived.parameters, auth=mReceived.authentication, json=mReceived.arguments, verify=False)
            numActualCalls += 1
            print("Num actual calls = " + str(numActualCalls))
            ...
            mapPIDtoIdle[mReceived.PID] += end - start
            for elemPID in funcCallTable[mReceived.url_func]:
                clientSocketCurr = mapPIDtoSocket[elemPID]
                clientSocketCurr.sendall(pickle.dumps(future))
                clientSocketCurr.close()
                numRunning = 0
                for index in range(len(requestQueue)):
                    if(mapPIDtoStatus[requestQueue[index]] == "running"):
                        numRunning += 1
                if numRunning >= numCores:
                    lastWaiting[elemPID] = float(time.time())
                    mapPIDtoStatus[elemPID] = "waiting"
                    psutil.Process(elemPID).suspend()
            del funcCallTable[mReceived.url_func]
        else:
            funcCallTable[mReceived.url_func].append(mReceived.PID)
            mapPIDtoSocket[mReceived.PID] = clientSocket
            mapPIDtoIdle[mReceived.PID] = 0
